...
with open('BPM_RAMOS_'+condicion+'_'+silencio+'.csv', 'w', encoding='UTF8') as f:
    ...
    for p in range(1,21):
      for t in range(1,4):
        for g in range(1,3):
          # carga del wav
          sr, data = scipy.io.wavfile.read('audios2410/P'+str(p)+'_'+condicion+'_'+silencio+'_GTR'+str(g)+'_00'+str(t)+'.wav')
          ...
          secciones = li.effects.split(data, top_db=28, frame_length=8192)  # top_db y frame_length son los parametros mas importantes
          #secciones = secciones[:-1]  # corta la ultima parte del numpyarray
          #secciones = np.delete(secciones, secciones.shape[0] - 1)
          ...
          for seccion in secciones:
            ...
            #medio = np.mean(bpm_medios)
            #bpm_prom.append(medio)
          ...
          #obtenemos e promedio del bpm de las cuatro ejecuciones sin contar los silencios
          #bpm_pro = sum(bpm_prom) / len(bpm_prom)

          print(str(p)+'_'+str(t)+'_'+str(g)+' BPM MEDIO:',np.mean(bpm_medios))
          print("Duración: ", (60 / np.mean(bpm_medios)) * 8, " Seg. ", " BPM: ", np.mean(bpm_medios))
